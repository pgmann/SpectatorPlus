    ...
    private static Location string2Location(String rawLocation) {
        ...
        try {
            location = new Location(
                    world,
                    Double.parseDouble(locationsPart[1]),
                    Double.parseDouble(locationsPart[2]),
                    Double.parseDouble(locationsPart[3])
            );

            if (locationsPart.length >= 5) {
                location.setPitch(Float.parseFloat(locationsPart[4]));

                if (locationsPart.length >= 6) {
                    location.setYaw(Float.parseFloat(locationsPart[5]));
                }
            }
