...
public class SpectatorsInventoryListener implements Listener {
    ...
    public void onInventoryClick(InventoryClickEvent ev) {
        ...
        if (SpectatorPlus.get().getPlayerData(clicker).isSpectating() && ev.getRawSlot() >= ev.getInventory().getSize()) {
    ...
    public void onPlayerPunchEntity(PlayerInteractEntityEvent ev) {
        if (ev.getRightClicked() instanceof Player && SpectatorPlus.get().getPlayerData(ev.getPlayer()).isSpectating()) {
            ItemStack mainItem = ev.getPlayer().getInventory().getItemInMainHand();
            if (mainItem.getItemMeta() != null && mainItem.getItemMeta().getDisplayName().startsWith(SpectatorsInventoryManager.INSPECTOR_TITLE)) {
    ...
    public void onPlayerPunchEntity(EntityDamageByEntityEvent ev) {
        if (ev.getDamager() instanceof Player && ev.getEntity() instanceof Player) {
            Player spec = (Player) ev.getDamager();
            ItemStack mainItem = spec.getInventory().getItemInMainHand();
            if (SpectatorPlus.get().getPlayerData(spec).isSpectating()
                    && mainItem.getItemMeta() != null && mainItem.getItemMeta().hasDisplayName()) {
                if (mainItem.getItemMeta().getDisplayName().startsWith(SpectatorsInventoryManager.INSPECTOR_TITLE)) {
                    handlePunch(spec, (Player) ev.getEntity());
                } else {
                    handleClick(spec, mainItem);
                }
            }
        }
    }
